...
const EndIcon = () => (
  <svg className="w-5.5 h-5.5" viewBox="0 0 24 24" fill="currentColor">
    <path
      d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.58.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1C10.29 21 3 13.71 3 4.5c0-.55.45-1 1-1H7.5c.55 0 1 .45 1 1 0 1.25.2 2.46.57 3.58.11.35.03.74-.24 1.02L6.6 10.8z"
...
export default function GoLive() {
  ...
  const addMsg = (text, self) =>
    setMsgs((prev) => [...prev, { text, self, time: new Date() }]);
  ...
  const flushPendingCandidates = async () => {
    if (
      ...
      console.log(
        `Flushing ${pendingCandidatesRef.current.length} pending ICE candidates`,
      );
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      const updateParticipants = () => {
        setParticipantCount(
          pcRef.current?.connectionState === "connected" ? 2 : 1,
        );
      };
      ...
      ws.onmessage = async (raw) => {
        ...
        switch (msg.type) {
          ...
          case "ready":
            ...
            targetUserRef.current = msg.userId || msg.from;
            setParticipantCount(
              pcRef.current?.connectionState === "connected" ? 2 : 1,
            );

            // CRITICAL FIX: Only ONE peer creates offer
            if (!connectionMadeRef.current && USER_ID < targetUserRef.current) {
          ...
          case "offer":
            ...
            if (!connectionMadeRef.current) {
              targetUserRef.current = msg.userId || msg.from || msg.target;
              setParticipantCount(2);
    ...
    <div className="h-screen flex flex-col bg-[#0c0e14] text-[#e2e5f0] select-none">
      ...
      <div className="h-13 bg-[#141720] border-b border-white/6 flex items-center justify-between px-5 shrink-0 z-20">
        <div className="flex items-center gap-2.5">
          <span className="font-mono text-sm font-medium text-[#4f8ef7] tracking-wide">
            vcall
          </span>
          <div className="w-px h-4.5 bg-white/11" />
          <span className="text-xs text-[#8b92a8] bg-[#1c2030] border border-white/6 px-2.5 py-0.5 rounded-full font-mono">
            #{ROOM}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <div
            className={`w-1.5 h-1.5 rounded-full ${wsStatus !== "connected" ? "bg-[#f87171] shadow-[0_0_6px_#f87171]" : peerUp ? "bg-[#34d399] shadow-[0_0_6px_#34d399] animate-pulse" : "bg-[#fbbf24] shadow-[0_0_6px_#fbbf24]"}`}
          />
          ...
          {peerUp && (
            <span className="font-mono text-xs text-[#525870] bg-[#1c2030] border border-white/6 px-2 py-0.5 rounded">
              {formatTime(callSecs)}
            </span>
          )}
        </div>
        <div className="flex gap-1.5">
          {["people", "chat"].map((p) => (
            <button
              ...
              className={`w-8.5 h-8.5 rounded-lg bg-[#1c2030] border border-white/6 text-[#8b92a8] cursor-pointer flex items-center justify-center transition-all duration-150 hover:bg-[#252a3d] hover:text-[#e2e5f0] hover:border-white/11 ${panel === p ? "!bg-[#4f8ef7] text-white! border-[#4f8ef7]" : ""}`}
            >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 relative bg-[#070910] overflow-hidden">
          ...
          {!peerUp && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-gradient-to-b from-[#141930] to-[#070910]">
              ...
              <span className="text-xs text-[#525870] font-mono bg-[#1c2030] border border-white/11 px-3.5 py-1 rounded-lg">
                Share room: {ROOM}
              </span>
            </div>
          )}
          ...
          <div className="absolute bottom-22 right-4.5 w-44 h-29 rounded-xl overflow-hidden border border-white/15 shadow-2xl bg-black transition-transform duration-180 hover:scale-105 z-5">
            {camOff && !screenOn && (
              <div className="absolute inset-0 bg-[#1c2030] flex items-center justify-center text-2xl">
                🙈
              </div>
            )}
            <video
            ...
            />
            <span className="absolute bottom-1.5 left-2 text-[10px] font-semibold text-white drop-shadow-lg">
              {screenOn ? "Screen" : "You"}
            </span>
          </div>
          ...
          <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-[#141720]/90 backdrop-blur-xl border border-white/11 px-4 py-2 rounded-full shadow-2xl z-10">
            {[
              ...
              {
                icon: <CamIcon off={camOff} />,
                active: !camOff,
                onClick: toggleCam,
                tooltip: camOff ? "Start Camera" : "Stop Camera",
              },
              {
                icon: <ScreenIcon active={screenOn} />,
                active: screenOn,
                onClick: toggleScreen,
                tooltip: screenOn ? "Stop Sharing" : "Share Screen",
                className: screenOn
              ...
              <div key={i} className="relative group">
                <button
                  onClick={btn.onClick}
                  className={`w-11 h-11 rounded-full flex items-center justify-center transition-all duration-160 ${btn.active ? "bg-[#252a3d] text-[#e2e5f0] border border-white/11 hover:bg-[#4f8ef7] hover:text-white hover:border-[#4f8ef7]" : "bg-[#f87171]/15 text-[#f87171] border border-[#f87171]/30 hover:bg-[#f87171] hover:text-white"} ${btn.className || ""}`}
                >
                  {btn.icon}
                </button>
            ...
            <div className="relative group">
              <button
                onClick={endCall}
                className="w-12.5 h-12.5 rounded-full bg-[#f87171] text-white flex items-center justify-center transition-all duration-160 hover:bg-[#ef4444] hover:scale-105 shadow-[0_4px_18px_rgba(248,113,113,0.38)]"
              >
                <EndIcon />
              </button>
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-[#252a3d] text-[#e2e5f0] border border-white/11 text-[11px] whitespace-nowrap px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-all duration-150 pointer-events-none translate-y-1 group-hover:translate-y-0">
        ...
        {/* Side Panel */}
        <div
          className={`w-75 bg-[#10131a] border-l border-white/6 flex flex-col overflow-hidden transition-all duration-220 shrink-0 ${!panel ? "!w-0" : ""}`}
        >
          {panel === "chat" && (
            <>
              <div className="px-4.5 py-3.5 border-b border-white/6 flex justify-between items-center shrink-0">
                ...
                <button
                  ...
                  className="bg-none border-none text-[#525870] cursor-pointer text-xl leading-5 px-1 py-0.5 rounded transition-colors duration-130 hover:text-[#e2e5f0]"
                >
                  ×
                </button>
              </div>
              <div className="flex-1 overflow-y-auto p-3.5 flex flex-col gap-2.5">
                ...
                {msgs.map((m, i) => (
                  <div
                    key={i}
                    className={`flex flex-col gap-0.5 ${m.self ? "items-end" : ""}`}
                  >
                    <div
                      className={`max-w-[82%] px-3 py-2 rounded-xl text-sm leading-relaxed break-words ${m.self ? "bg-[#4f8ef7] text-white rounded-br-md" : "bg-[#1c2030] text-[#e2e5f0] rounded-bl-md"}`}
                    >
                      {m.text}
                    </div>
                    <span className="text-[10.5px] text-[#525870] px-0.5">
                      {formatTimeHM(m.time)}
                    </span>
                  </div>
                ))}
              ...
              <div className="p-3 border-t border-white/6 flex gap-2 items-center shrink-0">
                <input
                  className="flex-1 bg-[#1c2030] border border-white/6 text-[#e2e5f0] text-sm px-3 py-2 rounded-lg outline-none transition-colors duration-140 focus:border-[#4f8ef7]"
                ...
                <button
                  onClick={sendChat}
                  disabled={!chatVal.trim()}
                  className="w-9 h-9 bg-[#4f8ef7] border-none rounded-lg text-white cursor-pointer flex items-center justify-center transition-all duration-160 hover:bg-[#3b7de8] hover:scale-105 disabled:bg-[#1c2030] disabled:text-[#525870] disabled:cursor-default disabled:transform-none shrink-0"
                >
                  <SendIcon />
                </button>
              </div>
            ...
            <>
              ...
              <div className="flex-1 p-3.5 flex flex-col gap-2">
                <div className="flex items-center gap-2.5 p-2.5 bg-[#1c2030] rounded-xl border border-white/6">
                  <div className="w-9 h-9 rounded-full bg-linear-to-br from-[#4f8ef7] to-[#a78bfa] flex items-center justify-center text-sm font-bold shrink-0">
                    Y
                  </div>
                  <div className="flex-1">
                    <div className="text-sm font-medium">You</div>
                    <div className="text-[11.5px] text-[#525870] mt-0.5">
                      {screenOn
                        ? "🖥 Sharing Screen"
                        : micMuted
                          ? "🔇 Muted"
                ...
                {peerUp && (
                  <div className="flex items-center gap-2.5 p-2.5 bg-[#1c2030] rounded-xl border border-white/6">
                    <div className="w-9 h-9 rounded-full bg-linear-to-br from-[#34d399] to-[#06b6d4] flex items-center justify-center text-sm font-bold shrink-0">
                      R
                    </div>
                    <div className="flex-1">
                      <div className="text-sm font-medium">Remote Peer</div>
                      <div className="text-[11.5px] text-[#525870] mt-0.5">
                        {remoteScreen ? "🖥 Sharing Screen" : "🎙 Connected"}
                      </div>
                    </div>
                    <div className="w-1.5 h-1.5 rounded-full bg-[#34d399]" />
                  </div>
